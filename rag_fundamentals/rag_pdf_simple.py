...
class SimpleModelSelector:
    ...
    def __init__(self):
        ...
        self.llm_models = {"gemma": "Gemma", "ollama": "Llama3"}

        # Available embedding model with their dimensions
        self.embedding_models = {
            "chroma": {
                "name": "Chroma Default", "dimensions": 384, "model_name": None
            },
            "nomic": {
...
class SimpleRAGSystem:
    ...
    def __init__(self, embedding_model="chroma", llm_model="gemma"):
    ...
    def query_documents(self, query, n_results=3):
        """Query documnets and return relevant chunks"""
        try:
            # Ensure collection exists
            if not self.collection:
            ...
            st.error(f"Error query document: {str(e)}")
            return None
    ...
    def generate_response(self, query, context):
        ...
        try:
            ...
            response = self.llm.chat.completions.create(
                model="gemma" if self.llm_model == "gemma" else "llama3",
                messages=[
...
def main():
    ...
    try:
        ...
        st.error(f"Eroor initilaizing RAG system: {str(e)}")
        return
        ...
        with st.spinner("Processing PDF..."):
            try:
                ...
                if st.session_state.rag_system.add_documents(chunks):
                    ...
                    st.success(f"Successfully porocessed {pdf_file.name}")
            except Exception as e:
                st.error(f"Eroor processing PDF: {str(e)}")
